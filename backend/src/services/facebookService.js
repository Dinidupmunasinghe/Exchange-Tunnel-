...
async function fetchFacebookPostOpenGraphPreview(inputUrl) {
  ...
  if (!/^https?:$/i.test(parsed.protocol) || !isAllowedFacebookPreviewHost(parsed.hostname)) {
    return { imageUrl: null, title: null, description: null, isVideo: false };
  }
...
async function verifyEngagement({ campaign, engagementType, proofText, verifiedViaFacebook }) {
  ...
  if (verifiedViaFacebook) {
    return {
      isValid: true,
      metaEngagementId: `fb-graph-${Date.now()}`,
      reason: "Verified via Facebook Graph API"
    };
  }
